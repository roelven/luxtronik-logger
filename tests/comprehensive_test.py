...
def run_docker_container(env_file, data_dir, duration=70):
    ...
    cmd = [
        "docker", "run",
        "--name", container_name,
        "--network", "host",  # Use host networking to access mock heat pump
        "--env-file", env_file,
    ...
    for i in range(duration):
        ...
        if result.returncode != 0 or result.stdout.strip() != "true":
            print(f"Container stopped after {i} seconds")
            break
        time.sleep(1)
        if i % 10 == 0:
            print(f"Container still running after {i} seconds...")
...
def run_comprehensive_test():
    ...
    try:
        ...
        sock.close()
        if result != 0:
            print("❌ Failed to verify mock heat pump is running")
            return False
        else:
            print("✅ Mock heat pump is running and accessible")
    except Exception as e:
        print(f"❌ Failed to verify mock heat pump: {e}")
        return False
